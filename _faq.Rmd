...
curation of the data. Regarding Hausa, consider the fact that the
[Glottolog](https://glottolog.org/) reference catalog has [more than 1400
references](https://glottolog.org/resource/languoid/id/haus1257) related to
Hausa, so perhaps it is unsurprising that Maddieson consulted 11 different
sources when preparing the Hausa entry for UPSID.
...
phoible %>% filter(InventoryID %in% sample_of_inventories_with_allophones) ->
    my_sample
```
