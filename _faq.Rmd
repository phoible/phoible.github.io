...
You can get the most recent official release from our [download page](https://phoible.org/download),
[view the data](https://github.com/phoible/dev/blob/master/data/phoible.csv) or
[directly download](https://github.com/phoible/dev/blob/master/data/phoible.csv?raw=true)
the most up-to-date version from GitHub, or use the following code snippet to
download the most up-to-date version directly from within `R`:
...
id_to_bibtex_mapping %>%
    ...
    select(references) %>%
    table() %>%
    as.data.frame(responseName="number of inventories") %>%
    select("number of doculects consulted"=".", "number of inventories") %>%
    kable()
```
